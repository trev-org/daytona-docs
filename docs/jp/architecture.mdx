...
Daytona provides **full composable computers** — [sandboxes](/docs/jp/sandboxes) — for AI agents. Daytona platform is organized into multiple plane components, each serving a specific purpose:
...
- **SDK**: [Python](/docs/jp/python-sdk), [TypeScript](/docs/jp/typescript-sdk), [Ruby](/docs/jp/ruby-sdk), and [Go](/docs/jp/go-sdk) for programmatic sandbox management
- [CLI](/docs/jp/tools/cli): command-line interface for direct sandbox operations
- [Dashboard](https://app.daytona.io/dashboard/): web interface for visual sandbox management and monitoring
- [MCP](/docs/jp/mcp): Model Context Protocol server for AI tool integration
- [SSH](/docs/jp/ssh-access): secure shell access to running sandboxes
...
- [Snapshot builder](#snapshot-builder) builds and manages sandbox [snapshots](/docs/jp/snapshots)
...
To interact with sandboxes from the API, see the [API](/docs/jp/tools/api) and [Toolbox API](/docs/jp/tools/api#daytona-toolbox) references.
...
The snapshot builder is part of the API process and orchestrates the creation of sandbox [snapshots](/docs/jp/snapshots) from a Dockerfile or a pre-built image from a [container registry](#container-registry). It coordinates with runners to build or pull images, which are then pushed to an internal snapshot registry that implements the OCI distribution specification.

#### Sandbox manager

The sandbox manager is part of the API process and schedules sandboxes onto runners, reconciles states, and enforces [sandbox lifecycle management](/docs/jp/sandboxes#sandbox-lifecycle) policies.

### Compute plane

The compute plane is the infrastructure layer where sandboxes run. Sandboxes run on [runners](#sandbox-runners), compute nodes that host multiple sandboxes with dedicated resources and scale horizontally across shared or dedicated [regions](/docs/jp/regions). The compute plane consists of the following components:
...
The sandbox daemon is a code execution agent that runs inside each sandbox. It exposes the [Toolbox API](/docs/jp/tools/api#daytona-toolbox), providing direct access to the sandbox environment: file system and Git operations, process and code execution, computer use, log streaming, and terminal sessions.
...
[Volumes](/docs/jp/volumes) provide persistent storage that can be shared across sandboxes. Each volume is backed by S3-compatible object storage and mounted into sandbox containers as a read-write directory. Multiple sandboxes can mount the same volume simultaneously, allowing data to be shared across sandboxes and persist independently of the sandbox lifecycle.

### Container registry

Container registries serve as the source for sandbox base images. When creating a [snapshot](/docs/jp/snapshots), the snapshot builder pulls the specified image from an external registry, and pushes it to the internal snapshot registry for use by runners. For Dockerfile-based snapshots, parent images referenced in `FROM` directives are also pulled from the configured source registries during the build. Daytona supports any OCI-compatible registry:

- [Docker Hub](/docs/jp/snapshots#docker-hub)
- [Google Artifact Registry](/docs/jp/snapshots#google-artifact-registry)
- [GitHub Container Registry (GHCR)](/docs/jp/snapshots#github-container-registry-ghcr)
- [Private registries](/docs/jp/snapshots#using-images-from-private-registries): any registry that implements the OCI distribution specification
