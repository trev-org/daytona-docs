...
Daytona supports managing your resources by [changing the state](/docs/en/sandboxes#sandbox-lifecycle) of your Sandboxes. The table below summarizes how each state affects resource usage:
...
This rate limit applies to all sandbox creation methods, including [creation from snapshots](/docs/en/snapshots#create-snapshots), [declarative builds](/docs/en/declarative-builder) and any other parameters passed to `daytona.create()` ([SDK](/docs/en/getting-started#sdks)) or POST requests to `/api/sandbox` ([API](/docs/en/getting-started#api)).
...
- [**Starting** sandboxes](/docs/en/tools/api#post-sandboxsandboxidornamestart) (`POST /api/sandbox/:id/start`)
- [**Stopping** sandboxes](/docs/en/tools/api#post-sandboxsandboxidornamestop) (`POST /api/sandbox/:id/stop`)
- [**Deleting** sandboxes](/docs/en/tools/api#delete-sandboxsandboxidorname) (`DELETE /api/sandbox/:id`)
- [**Archiving** sandboxes](/docs/en/tools/api#post-sandboxsandboxidornamearchive) (`POST /api/sandbox/:id/archive`)
...
Daytona [Python](/docs/en/python-sdk) or [TypeScript](/docs/en/typescript-sdk) SDKs raise or throw a `DaytonaRateLimitError` exception (Python) or error (TypeScript) when you exceed a rate limit.
...
For more information, see the [Python SDK](/docs/en/python-sdk/common/errors), [TypeScript SDK](/docs/en/typescript-sdk/errors), and [Go SDK](/docs/en/go-sdk/errors) references.

> [**DaytonaRateLimitError (Python SDK)**](/docs/en/python-sdk/common/errors#daytonaratelimiteerror)
>
> [**DaytonaRateLimitError (TypeScript SDK)**](/docs/en/typescript-sdk/errors#daytonaratelimiteerror)
>
> [**DaytonaRateLimitError (Go SDK)**](/docs/en/go-sdk/errors#daytonaratelimiteerror)
...
**Cache API responses** that don't frequently change, such as [sandbox lists](/docs/en/sandboxes#list-sandboxes) (when relatively static), [available regions](/docs/en/regions), and [snapshot information](/docs/en/snapshots). This reduces unnecessary API calls and helps you stay well within your limits.
...
**Efficiently manage sandbox lifecycle** to reduce API calls. [Archive sandboxes](/docs/en/sandboxes#archive-sandboxes) instead of deleting and recreating them, stop sandboxes when not in use rather than deleting them, and leverage [auto-stop intervals](/docs/en/sandboxes#auto-stop-interval) to automatically manage running sandboxes without manual intervention.
