...
const normalizeLocale = (locale: string) => (locale === 'ja' ? 'jp' : locale)

const SideNavLinksContent = ({ locale }: { locale: string }) => {
  const [stars, setStars] = useState<number | null | undefined>(undefined)
  const labels = LABELS[normalizeLocale(locale) as keyof typeof LABELS] ?? LABELS.en
...
export const SideNavLinks = ({ locale }: { locale: string }) => {
  return <SideNavLinksContent locale={locale} />
}
